Creep.prototype.powerDistro = function powerDistro() {  //TODO dont loop it all the time?
    ...
    let towersSum = _.sum(powerTowers, s => 'tower'[s]);
    ...
    let contisSum = _.sum(powerContis, s => 'conti'[s]);
    ...
    let exisSum = _.sum(powerExis, s => 'exi'[s]);
    ...
    let linksSum = _.sum(powerLinks, s => 'link'[s]);
    ...
    let spawnsSum = _.sum(powerSpawns, s => 'spawn'[s]);
    ...
    let storageSum = _.sum(powerStorage, s => 'storag'[s]); //Because only one per room,brutal much storage, LOWEST PRIO
